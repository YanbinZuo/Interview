...
### 4.内存中存放位置
 ...
 （2）堆（heap）：用于程序动态申请分配和释放空间。C语言中的malloc和free，C++中的new和delete均是在堆中进行的。正常情况下，程序员申请的空间在使用结束后应该释放，若程序员没有释放空间，则程序结束时系统自动回收。注意：这里的“堆”并不是数据结构中的“堆”。  new的实例放在堆里
...
### 6. TCP/IP    https://www.bilibili.com/video/BV1Vy4y177YB?p=2     
      TCP/IP: (客户端)      
        程序  应用层    
        内    传输层   
        核    网络层   
